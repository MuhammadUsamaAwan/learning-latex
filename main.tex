...
The function $f(x)=(x-3)^2+\frac{1}2{}$ has a domain of $\mathrm{D}_f:(-\infty, \infty)$ and the range of $\mathrm{R}_f:\left[\frac{1}2, \infty\right)$. % \mathrm{} removes italized
...
\begin{tikzpicture}
    \begin{axis}[xmin=-2, xmax=2, ymin=-2, ymax=2, xlabel=$x$, ylabel=$y$, title=My Graph]
        \addplot[color=red, dashed, mark=*, samples=50]{x^2};
...
\begin{tikzpicture}
    \begin{axis}[xmin=-2, xmax=2, ymin=-2, ymax=2, xlabel=$x$, ylabel=$y$, title=My Graph, axis lines=middle]
        \addplot[color=red, dashed, mark=*, samples=50]{x^2};
        \addplot[color=blue, samples=100, domain=-1:1]{1-x^2};
    \end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}
    \begin{axis}[clip=false, xmin=0, xmax=2.5*pi, ymin=-1.5, ymax=1.5, axis lines=middle, xtick={0,pi/2,pi,3*pi/2,2*pi}, xticklabels={$0$, $\frac{\pi}{2}$, $\pi$, $\frac{3\pi}{2}$, $2\pi$}, xticklabel style={anchor=south west}, xmajorgrids=true, grid style=dashed]
